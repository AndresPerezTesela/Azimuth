...
def featurize_data(data, learn_options, Y, gene_position, pam_audit=True, length_audit=True, quiet=True):
    ...
    if learn_options["include_gene_position"]:
        ...
        for _set in gene_position.columns:
            set_name = _set
            feature_sets[set_name] = pandas.DataFrame(gene_position[_set])
...
def check_feature_set(feature_sets):
    ...
    for _set in feature_sets.keys():
        if np.any(np.isnan(feature_sets[_set])):
            raise Exception("found Nan in set %s" % _set)
...
def normalize_feature_sets(feature_sets):
    ...
    for _set in feature_sets:
         new_feature_sets[_set] = normalize_features(feature__sets[_set],axis=0)
         if np.any(np.isnan(new_feature_sets[_set].values)):
             raise Exception("found Nan feature values in set=%s" % _set)
         assert new_feature_sets[_set].shape[1] > 0, "0 columns of features"
    t2 = time.time()
